...
function validarProponente(dados: Dados): Erros {
  const erros: Erros = {};
...
function validarProjeto(dados: Dados, faixa: { min: number; max: number }): Erros {
  const erros: Erros = {};
  ...
  if (!dados.valor.trim()) erros.valor = tc.erros.obrigatorio;
    ...
    erros.valor = `${tc.erros.valorFaixa} ${moeda(faixa.min)} a ${moeda(faixa.max)}.`;
  }
...
function validarPasso(passo: number, dados: Dados, faixa: { min: number; max: number }): Erros {
  if (passo === 0) return validarProponente(dados);
  if (passo === 1) return validarProjeto(dados, faixa);
  return {};
}
...
export function FormularioCandidatura({ edital }: { edital: EditalResumo }) {
  ...
  const faixaTexto = `${tc.projeto.faixaAjuda} ${moeda(faixa.min)} a ${moeda(faixa.max)}.`;
  ...
  function avancar() {
    const encontrados = validarPasso(passo, dados, faixa);
    setErros(encontrados);
    if (Object.keys(encontrados).length === 0) irPara(passo + 1);
  }

  function enviar() {
    const encontrados = { ...validarProponente(dados), ...validarProjeto(dados, faixa) };
    ...
    return (
      <section className="cartao" style={{ gap: 16 }}>
        ...
        <div className="linha" style={{ marginTop: 4 }}>
          <Botao href="/cadastro">{tc.cadastro.acao}</Botao>
          <Botao href={`/oportunidades/${edital.slug}`} variante="secundario">
            {tc.voltarEdital}
          </Botao>
        </div>
      </section>
      ...
      <div ref={topo} className="pilha--g">
        <section className="cartao" style={{ gap: 18 }}>
          ...
          <div>
            ...
            <Item rotulo={tc.confirmacao.situacao} valor={<Selo status={enviada.status} />} />
          </div>
        ...
        <div className="linha">
          <Botao href="/painel">{tc.confirmacao.acao}</Botao>
